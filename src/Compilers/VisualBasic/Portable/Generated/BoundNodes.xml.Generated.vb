...
Namespace Microsoft.CodeAnalysis.VisualBasic
    ...
    Friend NotInheritable Partial Class BoundConversion
        ...
        Public Sub New(syntax As SyntaxNode, operand As BoundExpression, conversionKind As ConversionKind, checked As Boolean, explicitCastInCode As Boolean, constantValueOpt As ConstantValue, extendedInfoOpt As BoundExtendedConversionInfo, isAsTypeConversion As Boolean, type As TypeSymbol, Optional hasErrors As Boolean = False)
            ...
            Me._IsAsTypeConversion = isAsTypeConversion
        ...
        Private ReadOnly _IsAsTypeConversion As Boolean
        Public ReadOnly Property IsAsTypeConversion As Boolean
            Get
                Return _IsAsTypeConversion
            End Get
        End Property
        ...
        Public Function Update(operand As BoundExpression, conversionKind As ConversionKind, checked As Boolean, explicitCastInCode As Boolean, constantValueOpt As ConstantValue, extendedInfoOpt As BoundExtendedConversionInfo, isAsTypeConversion As Boolean, type As TypeSymbol) As BoundConversion
            If operand IsNot Me.Operand OrElse conversionKind <> Me.ConversionKind OrElse checked <> Me.Checked OrElse explicitCastInCode <> Me.ExplicitCastInCode OrElse constantValueOpt IsNot Me.ConstantValueOpt OrElse extendedInfoOpt IsNot Me.ExtendedInfoOpt OrElse isAsTypeConversion <> Me.IsAsTypeConversion OrElse type IsNot Me.Type Then
                Dim result = New BoundConversion(Me.Syntax, operand, conversionKind, checked, explicitCastInCode, constantValueOpt, extendedInfoOpt, isAsTypeConversion, type, Me.HasErrors)
    ...
    Partial Friend NotInheritable Class BoundAsCast
        Inherits BoundExpression

        Public Sub New(syntax As SyntaxNode, operand As BoundExpression, conversionKind As ConversionKind, suppressVirtualCalls As Boolean, constantValueOpt As ConstantValue, relaxationLambdaOpt As BoundLambda, type As TypeSymbol, Optional hasErrors As Boolean = False)
            MyBase.New(BoundKind.AsCast, syntax, type, hasErrors OrElse operand.NonNullAndHasErrors() OrElse relaxationLambdaOpt.NonNullAndHasErrors())
        ...
        Public Overrides Function Accept(visitor As BoundTreeVisitor) As BoundNode
            Return visitor.VisitAsCast(Me)
        End Function

        Public Function Update(operand As BoundExpression, conversionKind As ConversionKind, suppressVirtualCalls As Boolean, constantValueOpt As ConstantValue, relaxationLambdaOpt As BoundLambda, type As TypeSymbol) As BoundAsCast
            If operand IsNot Me.Operand OrElse conversionKind <> Me.ConversionKind OrElse suppressVirtualCalls <> Me.SuppressVirtualCalls OrElse constantValueOpt IsNot Me.ConstantValueOpt OrElse relaxationLambdaOpt IsNot Me.RelaxationLambdaOpt OrElse type IsNot Me.Type Then
                Dim result = New BoundAsCast(Me.Syntax, operand, conversionKind, suppressVirtualCalls, constantValueOpt, relaxationLambdaOpt, type, Me.HasErrors)
    ...
    Partial Friend MustInherit Class BoundTreeVisitor(Of A, R)
        ...
        Friend Function VisitInternal(node As BoundNode, arg As A) As R
            Select Case node.Kind
                ...
                Case BoundKind.AsCast
                    Return VisitAsCast(CType(node, BoundAsCast), arg)
    ...
    Partial Friend MustInherit Class BoundTreeVisitor(Of A, R)
        ...
        Public Overridable Function VisitAsCast(node As BoundAsCast, arg As A) As R
            Return Me.DefaultVisit(node, arg)
        End Function
    ...
    Partial Friend MustInherit Class BoundTreeVisitor
        ...
        Public Overridable Function VisitAsCast(node As BoundAsCast) As BoundNode
            Return Me.DefaultVisit(node)
        End Function
    ...
    Partial Friend MustInherit Class BoundTreeWalker
        ...
        Public Overrides Function VisitAsCast(node As BoundAsCast) As BoundNode
            Me.Visit(node.Operand)
            Me.Visit(node.RelaxationLambdaOpt)
            Return Nothing
        End Function
    ...
    Partial Friend MustInherit Class BoundTreeRewriter
        ...
        Public Overrides Function VisitConversion(node As BoundConversion) As BoundNode
            ...
            Dim type As TypeSymbol = Me.VisitType(node.Type)
            Return node.Update(operand, node.ConversionKind, node.Checked, node.ExplicitCastInCode, node.ConstantValueOpt, extendedInfoOpt, node.IsAsTypeConversion, type)
        End Function
    ...
    Friend NotInheritable Class BoundTreeDumperNodeProducer
        ...
        Public Overrides Function VisitConversion(node As BoundConversion, arg As Object) As TreeDumperNode
            Return New TreeDumperNode("conversion", Nothing, New TreeDumperNode() {
                ...
                New TreeDumperNode("isAsTypeConversion", node.IsAsTypeConversion, Nothing),
                New TreeDumperNode("type", node.Type, Nothing)
            })
        End Function
        ...
        Public Overrides Function VisitAsCast(node As BoundAsCast, arg As Object) As TreeDumperNode
            Return New TreeDumperNode("asCast", Nothing, New TreeDumperNode() {
                New TreeDumperNode("operand", Nothing, new TreeDumperNode() { Visit(node.Operand, Nothing) }),
                New TreeDumperNode("conversionKind", node.ConversionKind, Nothing),
                New TreeDumperNode("suppressVirtualCalls", node.SuppressVirtualCalls, Nothing),
                New TreeDumperNode("constantValueOpt", node.ConstantValueOpt, Nothing),
